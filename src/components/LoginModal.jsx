...
const LoginModal = ({ isOpen, onClose, onLoginSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const customerName = location.pathname.split('/').filter(Boolean)[0] || 'hanssem';
        const API_BASE_URL = import.meta.env.VITE_API_BASE_URL;

        try {
            const response = await fetch(`${API_BASE_URL}/auth/${customerName}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok && result.status === 'success') {
                // 토큰을 쿠키의 Authorization 헤더 형식으로 저장 (7일 유지)
                Cookies.set('Authorization', result.token, { expires: 7 });

                alert('반갑습니다! 성공적으로 로그인되었습니다.');
                onLoginSuccess();
            } else {
                setError(result.message || '로그인에 실패했습니다. 비밀번호를 확인해주세요.');
            }
        } catch (err) {
